...
(function ($, window, document, undefined) {
  ...
  $( document ).ready(function() {
    ...
    $('#nb-menu-link').on('click',function(){
      var disp = $('#nb-popoutmenu').css('display');
      var margin =  $('#nb-popoutmenu').css('transform');
      if(disp == 'none'){
        $('#nb-popoutmenu').css('display','block').css('width','260px').css('transform','matrix(1, 0, 0, 1, 0, 0)').css('transition','transform 1s');
        $('#nb-menu-page').css('display','block').css('transform','matrix(1, 0, 0, 1, 260, 0)').css('transition','transform 1s');
        $('.top-nav').css('transform','matrix(1, 0, 0, 1, 260, 0)');
      }
      else if(disp == 'block' && margin == 'matrix(1, 0, 0, 1, -260, 0)'){
        $('#nb-popoutmenu').css('transform','matrix(1, 0, 0, 1, 0, 0)').css('transition','transform 1s');
        $('#nb-menu-page').css('display','block').css('transform','matrix(1, 0, 0, 1, 260, 0)').css('transition','transform 1s');
        $('.top-nav').css('transform','matrix(1, 0, 0, 1, 260, 0)');
      }
      else{
        $('#nb-popoutmenu').css('transform','matrix(1, 0, 0, 1, -260, 0)').css('transition','transform 1s');
        $('#nb-menu-page').css('transform','matrix(1, 0, 0, 1, 0, 0)').css('transition','transform 1s');
        $('.top-nav').css('transform','matrix(1, 0, 0, 1, 0, 0)').css('transition','transform 1s');
      }
  });
  })
...
(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    _initVars: function() {
      ...
      base.$lastSection 		= $('#fullpager').find('.section:last-child'); // last section
      base.$vItems 			= base.$el.find('.v-item'); // animation items
      base.$contentSection 	= $('.page-content-wrapper');
      base.$fullpageWrapper 	= $('#fullpager-wrapper');
    ...
    beforeInit: function() {
      ...
      this.$b.removeClass('fullpage-not-showing');
    ...
    loadFullPage: function() {
      ...
      if ($.fn.fullpage) {
        ...
        base.$el.fullpage({
          ...
          slideSelector: '.slide',
          // well do device detection // base.options.responsive && base.options.autoScrolling ? base.options.responsive : 0,
    ...
    onEnd: function(end) {
      ...
      if (end == 'bottom' &&  ($(".page-content-wrapper .page-content-inner").length > 0)) {
    ...
    slideFullPageUp: function(silent) {
      ...
      if ( base.options.autoScrolling ) {
        ...
        base.$b.addClass('fullpage-not-showing');
    ...
    _setNavPadding: function() {
      ...
      var HtconWidth = $('.text-cell').innerWidth();


      $('.text-layout-inner').css({
        'width': HtconWidth + 'px'
      });
      ...
      $('.section-entry .text-cell').css({
        /*'padding-bottom': topHeight/2 + 'px'*/
      });


      $('#fullpager .section .image-container').css({
        'height': sHeight/2 + (sHeight/10) + 'px',
      });


      $('.section-entry').css({
        'padding-top': (topHeight * 1.5) + 'px',
        'padding-bottom': 0 + 'px'
      ...
      if($(window).outerWidth() < 992){
        $('#section-1 .section-entry').css({
          'padding-top': sHeight/3 + 'px',
        });
      }

      /*$('#bigNumbers').css({
       'margin-top': (topHeight * 1.5) + 'px',
       ...
       $('.marTopBottom').css({
       'margin-top': (topHeight * 1.5) + 'px',
       'margin-bottom':(topHeight * 1.5) + 'px'
       });*/
    ...
    _overlayAnimations: function(items) {
      ...
      // animation elements
        ...
        title 		= items.find('.v-item.title, .v-item.owl-controls'),
        subtitle 	= items.find('.v-item.subtitle'),
        content 	= items.find('.v-item.content, .v-item.leadPara, .v-item.subtitle2, .v-item.animateImage, .v-item.tw-tabs-shortcode, .v-item.isotope-wrapper'),
...
(function ($) {
  ...
  $.fn.themewichBackgroundDetect = function() {
    ...
    if ( $('#fullpager').hasClass('scrolling-auto') ) {
  ...
  $(document).ready(function() {
    ...
    $('#ui-id-1').click(function(){
      $('.pss').removeClass('current');
      $("#parentTitle").addClass('current');
    });

    $('#ui-id-2').click(function(){
      $('.pss').removeClass('current');
      $("#studentTitle").addClass('current');
    });

    $('#ui-id-3').click(function(){
      $('.pss').removeClass('current');
      $("#schoolTitle").addClass('current');
    });

    if(!$(".page-content-wrapper .page-content-inner").length > 0)
    {
      $('a.go-to-content, .nextbutton.last-section').remove();
    }

    if($(window).outerWidth() > 767){
      if($('.fullWinConDiv').length) {
        var postWidth 	= $('.layout-center .text-cell').outerWidth(),
          windowWidth = $(window).width(),
          padding 	= ((windowWidth - postWidth)/2) + 15;
      ...
      var windowHeightaj = $(window).height();
      $('.page-align-left.left-title.page-content-wrapper.no-sections.template-grid.type-page').css({
        'min-height': windowHeightaj + 'px',
      });


      $("#ninja_forms_field_31").change(function() {
        var val = $(this).val();
        if (val == "Parent" || val == "Other") {
          $("#ninja_forms_field_30").html("<option value=''>I'm interested in *</option><option value='Recommending XSEED to a school'>Recommending XSEED to a school</option><option value='Giving Feedback'>Giving Feedback</option><option value='General Enquiry'>General Enquiry</option>");
        } else if (val == "School Owner") {
          $("#ninja_forms_field_30").html("<option value=''>I'm interested in *</option><option value='Buying XSEED for a school'>Buying XSEED for a school</option><option value='Giving feedback'>Giving feedback</option><option value='General Enquiry'>General Enquiry</option>");
        } else if (val == "Professional") {
          $("#ninja_forms_field_30").html("<option value=''>I'm interested in *</option><option value='Working with XSEED'>Working with XSEED</option><option value='Referring a friend'>Referring a friend</option><option value='General Enquiry'>General Enquiry</option>");
        } else if (val == "Media Person" || val == "Blogger") {
          $("#ninja_forms_field_30").html("<option value=''>I'm interested in *</option><option value='Finding out more about XSEED'>Finding out more about XSEED</option><option value='Writing about XSEED'>Writing about XSEED</option><option value='General Enquiry'>General Enquiry</option>");
        }
      });
    ...
    var highestBox = 0;
    $('#visionPhil .tw-column', this).each(function(){

      if($(this).height() > highestBox)
        highestBox = $(this).height();
    });

    $('#visionPhil .tw-column').height(highestBox);
...
//$.fn.fullpage.setAutoScrolling(false);
        ...
        //var bgImage = $('.home #section-1 .image-container.full-h-w-i-b.v-item.wp-caption').css('background-image');
        //$('.home .section .image-container.full-h-w-i-b.v-item.wp-caption').css('background-image', 'none');
        //$('#section-1').css('background-image', bgImage );
    ...
    if (type=="buyXseed")
    {
      $(window).load(function() {
        ...
        $('[name=ninja_forms_field_31]').val( "School Owner" );
        $("#ninja_forms_field_30").html("<option value=''>I'm interested in *</option><option value='Buying XSEED for a school'>Buying XSEED for a school</option><option value='Giving feedback'>Giving feedback</option><option value='General Enquiry'>General Enquiry</option>");
        $('[name=ninja_forms_field_30]').val( "Buying XSEED for a school" );
    ...
    if (type=="writeXseed")
    {
      $(window).load(function() {
        $("html, body").animate({ scrollTop: $(document).height()-$(window).height() }, 500);
        $('[name=ninja_forms_field_31]').val( "Media Person" );
        $("#ninja_forms_field_30").html("<option value=''>I'm interested in *</option><option value='Finding out more about XSEED'>Finding out more about XSEED</option><option value='Writing about XSEED'>Writing about XSEED</option><option value='General Enquiry'>General Enquiry</option>");
        $('[name=ninja_forms_field_30]').val( "Writing about XSEED" );
        $( "#ninja_forms_field_24" ).focus();
      });
    }
